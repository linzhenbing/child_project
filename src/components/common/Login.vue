              ...
              <div class="hero-copy">
                <div class="container-sm">
                  ...
                  <div class="input_username">
                    <span class="tips hidden" ref="tips">用户名或密码错误！</span>
...
<script>
  ...
  export default {
    ...
    //判断用户是否已经登陆过，如果登陆过，直接跳转到用户管理页
    created(){
      if(this.$store.getters.isLogin === 'true'){
        this.$router.push('/mdb/user')
      }
    },
    methods:{
      login(){
        ...
        if(this.username.length===0){
          ...
          this.$http.post('/user/login',{ username:this.username,password:this.password,type:this.type }).then(res=>{
            ...
            if(res.data.message === "用户名或密码错误"){
              ...
              tips.classList.remove("show")
              tips.classList.add("show")
            }else if(res.data.message === "用户被禁用"){
              tips.innerHTML = "用户被禁用！"
              tips.classList.remove("hidden")
              tips.classList.remove("show")
              tips.classList.add("show")
            }else{
...
<style scoped>
  .tips{ font-size: 14px;color: #dd1300; }
  .hidden{ display: none; }
  .show{ display: block; }
  .input_username,.input_password{ margin-top: 40px;  }
  ...
  .button_login,.button_register{ border: none;width: 150px;padding: 3px 0;outline: none;margin-top: 20px;font-size: 18px;cursor: pointer;border-radius: 3px; }
  .button_login{ background: linear-gradient(to left, rgb(155,149,243) 100%,rgb(230,229,255) 100%);color: white;box-shadow: 0 0 15px rgba(155, 149, 243, 0.58); }
  .button_register{ margin-top: 20px;margin-left: 20px;display: inline-block;background-color: rgb(240,240,240);color: black;font-weight: normal;box-shadow: 0 0 15px rgb(240,240,240); }
