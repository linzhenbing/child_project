            ...
            <div class="hero-inner">
              <div class="hero-copy">
                <div class="container-sm">
                  <h2>多媒体管理系统</h2>
                  <span class="tips hidden" ref="tips">请输入用户名</span>
                  <div class="input_username">
                    <span>&nbsp;&nbsp;用户名&nbsp;&nbsp;：</span><input type="text" id="username" v-model="username">
                  </div>
                  <div class="input_password">
                    <span>&nbsp;&nbsp;密&nbsp;&nbsp;&nbsp;码&nbsp;&nbsp;：</span><input type="password" style="margin-left: 2px;" id="password" v-model="password" readonly onfocus="this.removeAttribute('readonly');">
                  </div>
                  <div class="input_password">
                    <span>确认密码：</span><input type="password" style="margin-left: 2px;" id="check_password" v-model="check_password" readonly onfocus="this.removeAttribute('readonly');">
                  </div>
                  ...
                  <button class="button_register" @click="register">注册</button><br>
                  <router-link to="/login" class="toLogin">已有账号？点击登陆</router-link>
                </div>
              </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      register(){
        //获取type
        let plain = this.$refs.plain
        if (plain.checked === true) {
          this.type = plain.value
        }else {
          this.type = this.$refs.super.value
        }
        ...
        if (this.username.length === 0 ) {
          ...
          this.$http.post('/user/registered',{ username:this.username,password:this.password,type:this.type }).then(res=>{
...
<style scoped>
  ...
  .tips{ color: #dd1300; }
  .input_password{ margin-top: 30px;  }
  .input_username input,.input_password input{ border: none;border-bottom: 1px solid #999;outline: none;padding: 0 5px;font-weight: lighter
  ;font-size: 16px;width: 220px;}
  .input_username span,.input_password span{ font-size: 16px; }

  .button_register{ border: none;width: 290px;padding: 3px 0;outline: none;margin-top: 20px;font-size: 18px;cursor: pointer;background-color: rgb(155,149,243);color: white;border-radius: 3px;box-shadow: 0 0 15px rgba(155, 149, 243, 0.58); }
  .toLogin{ margin-top: 15px;display: inline-block; }
  .selectType label{ font-size: 14px;margin-top: 40px; }
  .selectType label input{ margin-right: 2px;vertical-align: middle; }
  .selectType label:first-child{ margin-right: 70px; }
